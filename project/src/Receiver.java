...
public class Receiver {
    ...
    public static void main(final String[] args) {
        ...
        int data_size = (Config.FRAME_SIZE+Config.CRC_SIZE +Config.ID_SIZE);
            ...
            try {
                ...
                if (data_signal.size() == Config.FRAME_SAMPLE_SIZE + 8){
                    ...
                    for (int i = 0; i < Config.SAMPLE_PER_BIT * data_size; i++) {
                        data_signal_remove_carrier[i] = data_signal.get(i + 4) * carrier.get(i);
                    }
                    ...
                    // decode ack
                    // remove carrier
                    for (int i = 0; i < Config.SAMPLE_PER_BIT * Config.ACK_SIZE; i++) {
                        data_signal_remove_carrier[i] = data_signal.get(i + 4) * carrier.get(i);
                    }

                    // decode
                    decoded_data_foreach.clear();
                    for (int i = 0; i < Config.ACK_SIZE; i++) {
                        sum = 0.0f;
                        for (int j = i * Config.SAMPLE_PER_BIT; j < (i + 1) * Config.SAMPLE_PER_BIT; j++) {
                            sum += data_signal_remove_carrier[j];
                        }
                    ...
                    // check crc code
                    List<Integer> transmitted_crc = decoded_data_foreach.subList(Config.ID_SIZE, Config.ACK_SIZE);
                    List<Integer> calculated_crc = CRC8.get_crc8(decoded_data_foreach.subList(0, Config.ID_SIZE));
                    ...
                    if(!transmitted_crc.equals(calculated_crc)){
                        System.out.println("ACK CRC check fails at idx = " + id);
                        boolean correct = false;
                        int offset;
                        for(offset = 0; offset < 8; offset++)
                        {
                            ...
                            // check crc code
                            transmitted_crc = decoded_data_foreach.subList(Config.ID_SIZE, Config.ACK_SIZE);
                            calculated_crc = CRC8.get_crc8(decoded_data_foreach.subList(0, Config.ID_SIZE));
                        ...
                        if(correct){
                            System.out.println("Fixed ACK idx = " + id + " with offset = " + offset);
                        }
                        else{
                            System.out.println("Couldn't fix ACK idx = " + id);
                        }
                    }
